...
import { ONE, ZERO, Q96, SOLIDITY_TYPE_MAXIMA, SolidityType } from '../../constants'
...
function multiplyIn256(x: JSBI, y: JSBI): JSBI {
  ...
  return JSBI.bitwiseAnd(product, SOLIDITY_TYPE_MAXIMA[SolidityType.uint256])
}

function addIn256(x: JSBI, y: JSBI): JSBI {
  const sum = JSBI.add(x, y)
  return JSBI.bitwiseAnd(sum, SOLIDITY_TYPE_MAXIMA[SolidityType.uint256])
}
