...
describe('ConcentratedPool', () => {
  ...
  describe('constructor', () => {
    it('cannot be used for tokens on different chains', () => {
      expect(() => {
        new ConcentratedPool(USDC, WETH9[3], FeeAmount.MEDIUM, encodeSqrtRatioX96('1', '1'), '0', 0, [])
      }).toThrow('CHAIN_IDS')
    })

    it('fee must be integer', () => {
      expect(() => {
        new ConcentratedPool(USDC, WETH9[1], FeeAmount.MEDIUM + 0.5, encodeSqrtRatioX96('1', '1'), '0', 0, [])
      }).toThrow('FEE')
    })

    it('fee cannot be more than 1e6', () => {
      expect(() => {
        new ConcentratedPool(USDC, WETH9[1], 1e6, encodeSqrtRatioX96('1', '1'), '0', 0, [])
      }).toThrow('FEE')
    })

    it('cannot be given two of the same token', () => {
      expect(() => {
        new ConcentratedPool(USDC, USDC, FeeAmount.MEDIUM, encodeSqrtRatioX96('1', '1'), '0', 0, [])
      }).toThrow('ADDRESSES')
    })

    it('price must be within tick price bounds', () => {
      expect(() => {
        new ConcentratedPool(USDC, WETH9[1], FeeAmount.MEDIUM, encodeSqrtRatioX96('1', '1'), '0', 1, [])
    ...
    it('works with valid arguments for empty pool medium fee', () => {
      new ConcentratedPool(USDC, WETH9[1], FeeAmount.MEDIUM, encodeSqrtRatioX96('1', '1'), '0', 0, [])
    })

    it('works with valid arguments for empty pool low fee', () => {
      new ConcentratedPool(USDC, WETH9[1], FeeAmount.LOW, encodeSqrtRatioX96('1', '1'), '0', 0, [])
    })

    it('works with valid arguments for empty pool lowest fee', () => {
      new ConcentratedPool(USDC, WETH9[1], FeeAmount.LOWEST, encodeSqrtRatioX96('1', '1'), '0', 0, [])
    })

    it('works with valid arguments for empty pool high fee', () => {
      new ConcentratedPool(USDC, WETH9[1], FeeAmount.HIGH, encodeSqrtRatioX96('1', '1'), '0', 0, [])
    })
  })

  describe('#getAddress', () => {
    it('matches an example', () => {
      const result = ConcentratedPool.getAddress(USDC, DAI, FeeAmount.LOW)
  ...
  describe('#token0', () => {
    it('always is the token that sorts before', () => {
      let pool = new ConcentratedPool(USDC, DAI, FeeAmount.LOW, encodeSqrtRatioX96('1', '1'), '0', 0, [])
      expect(pool.token0).toEqual(DAI)
      pool = new ConcentratedPool(DAI, USDC, FeeAmount.LOW, encodeSqrtRatioX96('1', '1'), '0', 0, [])
      expect(pool.token0).toEqual(DAI)
    })
  })
  describe('#token1', () => {
  ...
  describe('#priceOf', () => {
    const pool = new ConcentratedPool(USDC, DAI, FeeAmount.LOW, encodeSqrtRatioX96('1', '1'), '0', 0, [])
  ...
  describe('swaps', () => {
    let pool: ConcentratedPool

    beforeEach(() => {
      pool = new ConcentratedPool(USDC, DAI, FeeAmount.LOW, encodeSqrtRatioX96('1', '1'), ONE_ETHER, 0, [
        {
  ...
  describe('#bigNums', () => {
    ...
    beforeEach(() => {
      pool = new ConcentratedPool(USDC, DAI, FeeAmount.LOW, encodeSqrtRatioX96(bigNum1, bigNum2), ONE_ETHER, 0, [
        {
          index: nearestUsableTick(TickMath.MIN_TICK, TICK_SPACINGS[FeeAmount.LOW]),
          liquidityNet: ONE_ETHER,
          liquidityGross: ONE_ETHER
